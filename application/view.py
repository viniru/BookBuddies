...
def book():
    ...
    cur.execute('''SELECT title, rating, no_ratings,  description FROM Book WHERE b_id={0}'''.format(int(b_id)))
    vr = cur.fetchall()
    ...
    cur.execute('''SELECT Author.a_id, name FROM Author inner join AuthorBooks on Author.a_id = AuthorBooks.a_id WHERE AuthorBooks.b_id={0}'''.format(b_id))
    vr = cur.fetchall()
    response['Author'] = vr


    cur.execute('''SELECT c_id, title, likes, u_id FROM Comments WHERE b_id={0}'''.format(b_id))
    vr = cur.fetchall()
    response['Comments'] = vr


    cur.execute('''SELECT g_id, name FROM Genre WHERE g_id in (SELECT g_id FROM GenreBooks WHERE b_id={0})'''.format(b_id))
    vr = cur.fetchall()
